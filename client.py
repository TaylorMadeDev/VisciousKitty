...
def submit_task(task_id: str, result: str):
    machine_id = get_machine_id()
    try:
        response = requests.post(f"{SERVER_URL}submittask", params={"machine_id": machine_id, "task_id": task_id, "result": result}, timeout=5)
        try:
            print(response.text)
        except Exception:
            pass
...
def main():
    try:
        while not STOP_EVENT.is_set():
            ...
            # wait can be interrupted by signals; returns True if event set
            STOP_EVENT.wait(sleep_duration)
            if STOP_EVENT.is_set():
                break
            check_in()
            # Fetch and execute any pending tasks for this machine
            tasks = get_tasks()
            if tasks:
                for t in tasks:
